    ...
    <div class="card border-0 shadow-sm rounded-3">
        <div class="card-header bg-white border-bottom pt-3 d-flex justify-content-between align-items-center">
            <h5 class="mb-0 fw-bold text-dark"><i
                    class="fas fa-shopping-cart text-primary me-2"></i>{{ __('Stock Purchase History') }}</h5>
        ...
        <div class="card-body bg-light rounded-bottom">
            ...
            <form action="{{ route('purchases.index') }}" method="GET" class="row w-100 g-2 mb-4 d-print-none">
                <div class="row g-3 align-items-end">
                    <div class="col-md-4">
                        ...
                        <div class="input-group shadow-sm">
                            ...
                            <input type="text" name="search" id="search"
                                ...
                                placeholder="{{ __('Supplier name, total amount...') }}" value="{{ request('search') }}">
                        </div>
                    </div>

                    <div class="col-md-3">
                        <label for="start_date"
                            class="form-label small fw-bold text-muted mb-1">{{ __('inventory.filter_start_date') }}</label>
                    ...
                    <div class="col-md-3">
                        <label for="end_date"
                            class="form-label small fw-bold text-muted mb-1">{{ __('inventory.filter_end_date') }}</label>
                    ...
                    <div class="col-md-2">
                        <div class="d-flex gap-2">
                            ...
                            @if (request()->anyFilled(['search', 'start_date', 'end_date']))
                                <a href="{{ route('purchases.index') }}" class="btn btn-light border shadow-sm"
            ...
            <div
                class="alert alert-info border-info-subtle shadow-sm d-flex justify-content-between align-items-center mb-4 py-3">
                ...
                <table class="table table-hover align-middle mb-0">
                    <thead class="table-light text-secondary small text-uppercase">
                        <tr>
                            ...
                            <th class="text-start pe-3 py-3"> <button onclick="window.print()"
                        ...
                        @forelse($purchases as $purchase)
                            <tr>
                                <td class="ps-3 fw-medium text-muted">
                                    PO-{{ str_pad($purchase->PurchaseID, 5, '0', STR_PAD_LEFT) }}</td>
                                ...
                                <td><span
                                        class="badge bg-success bg-opacity-10 text-success border border-success-subtle px-2 py-1"><i
                                            class="fas fa-check-circle me-1"></i> {{ __('Completed') }}</span></td>
