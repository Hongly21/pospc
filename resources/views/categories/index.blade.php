...
@section('content')
    ...
    <div class="card border-0 shadow-sm rounded-3">
        <div class="card-header bg-white border-bottom pt-3 pb-3 d-flex justify-content-between align-items-center">
            <h5 class="mb-0 fw-bold text-dark"><i
                    class="fas fa-folder-open text-primary me-2"></i>{{ __('categories.list_header') }}</h5>
                                ...
                                <td class="text-center">
                                    @if ($category->status == 1)
                                        <span
                                            ...
                                            {{ __('categories.status_active') }}</span>
                                    @else
                                        <span
                                            ...
                                            {{ __('categories.status_inactive') }}</span>
                                    @endif
                                </td>
                                    ...
                                    <div class="btn-group shadow-sm">
                                        ...
                                        @if ($category->products_count > 0)
                                            <button type="button" class="btn btn-sm btn-light text-secondary border"
                                                disabled title="{{ __('categories.msg_cannot_delete') }}">
                                ...
                                <td colspan="4" class="text-center py-5 text-muted bg-white">
                                    <div class="d-flex flex-column align-items-center justify-content-center py-4">
                                        ...
                                        <h5 class="fw-medium text-dark">
                                            {{ __('categories.no_data') ?? 'No categories found.' }}</h5>
                                    </div>
                                </td>
                            </tr>
                        @endforelse
    ...
    <div class="modal fade" id="addCategoryModal" tabindex="-1">
        <div class="modal-dialog modal-dialog-centered modal-lg">
            <div class="modal-content border-0 shadow">
                <div class="modal-header bg-light border-bottom-0">
                    <h5 class="modal-title fw-bold text-dark"><i
                            class="fas fa-plus-circle text-primary me-2"></i>{{ __('categories.btn_add') }}</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                </div>
                <form action="{{ route('categories.store') }}" method="POST">
                    ...
                    <div class="modal-body p-4">
                        <div class="row g-3">
                            <div class="col-12">
                                <label class="form-label small fw-bold text-muted">{{ __('categories.category_name') }}
    ...
    @push('scripts')
        <script>
            window.categoriesIndexConfig = {
                swalTitle: "{{ __('categories.swal_delete_title') }}",
                swalText: "{{ __('categories.swal_delete_text') }}",
                swalConfirmBtn: "{{ __('categories.swal_confirm_btn') }}",
