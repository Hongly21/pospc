...
@section('content')
                ...
                <div class="card-header bg-white py-3">
                    <form action="{{ route('pos.index') }}" method="GET" class="row g-2 align-items-center mb-3">
                        ...
                        <div class="col-12 col-md-4 d-flex gap-2">
                            <button type="submit" class="btn btn-outline-primary px-4 flex-grow-1">{{ __('pos.search_btn') }}</button>
                            ...
                            <div class="col-md-3 col-6 product-card">
                                <div class="card h-100 border-0 shadow-sm btn-add-cart" style="cursor: pointer;"
                                    ...
                                    <div class="card-body text-center p-2">
                                        ...
                                        <div class="text-primary">{{ __('pos.price') }}: ${{ number_format($product->SellPrice, 2) }}</div>
                                        <small class="text-muted">{{ __('pos.stock') }}: {{ $product->inventory->Quantity }}</small>
                                    </div>
                                </div>
                            </div>
                        @endforeach
                            ...
                            <select class="form-select" id="customer_id">
                                ...
                                @foreach ($customers as $customer)
                                    @if ($customer->has_debt)
                                        <option value="{{ $customer->CustomerID }}" style="color:#dc3545;font-weight:bold;">
        ...
        <div class="modal-dialog modal-dialog-centered modal-sm">
            <div class="modal-content">
                ...
                <div class="modal-footer p-2">
                    <button type="button" class="btn btn-outline-primary btn-sm w-100" id="btn_save_customer">{{ __('pos.save') }}</button>
                </div>
            </div>
        </div>
    </div>
    ...
    $(document).ready(function () {
        ...
        const Toast = Swal.mixin({
            ...
            showConfirmButton: false, timer: 900, timerProgressBar: true,
        ...
        $(".btn-add-cart").click(function () {
            ...
            if (stock <= 0) {
                Toast.fire({ icon: "error", title: "{{ __('pos.out_of_stock') }}" }); return;
            }

            let item = cart.find(i => i.id === id);
            if (item) {
                if (item.qty >= stock) {
                    Toast.fire({ icon: "warning", title: `${name} {{ __('pos.stock_limit') }} ${stock}` }); return;
                }
                item.qty++;
                Toast.fire({ icon: "success", title: "{{ __('pos.added') }}" });
            } else {
                cart.push({ id, name, price, qty: 1, stock });
                Toast.fire({ icon: "success", title: "{{ __('pos.add_product') }}" });
            }
            renderCart();
        });
        ...
        $('#btn_save_customer').click(function () {
            ...
            if (!name || !phone) {
                Toast.fire({ icon: "warning", title: "Please fill in all fields" }); return;
            }
            $.ajax({
                url: "{{ route('customers.store.ajax') }}", type: "POST",
                data: { name, phone, _token: "{{ csrf_token() }}" },
                success(res) {
                    bootstrap.Modal.getInstance(document.getElementById('addCustomerModal'))?.hide();
                    $('#customer_id').append(new Option(`${res.name} (${res.phone})`, res.id, true, true));
        ...
        $('#customer_id').change(function () {
            ...
            $.get(`/pos/customer-debt/${id}`, function (res) {
                if (res.has_debt) {
                    Swal.fire({ title: 'អតិថិជននេះមានជំពាក់ប្រាក់!', text: res.message,
                        icon: 'warning', confirmButtonText: 'យល់ព្រម', confirmButtonColor: '#d33' });
                }
            });
        });
        ...
        $("#btnCheckout").click(function () {
            if (cart.length === 0) {
                Toast.fire({ icon: "warning", title: "មិនមានទំនិញ" }); return;
            }
        ...
        function startKhqrFlow(amount) {
            ...
            $.ajax({
                url: "{{ route('pos.khqr.generate') }}", type: "POST",
                data: { _token: "{{ csrf_token() }}", amount: amount, currency: 'USD' },
                success(res) {
                ...
                error() { showQrError('Server error — could not generate QR'); }
            });
        }
        ...
        function startPolling() {
            ...
            qrPollingTimer = setInterval(function () {
                if (!qrMd5) return;
                $.ajax({
                    url: "{{ route('pos.khqr.check') }}", type: "POST",
                    data: { _token: "{{ csrf_token() }}", md5: qrMd5 },
                    success(res) {
                        if (res.paid) {
                            onQrPaid();
                        }
        ...
        $("#btnConfirmPayment").click(function () {
            ...
            if (payType === 'QR') {
                if (!qrConfirmed) {
                    Toast.fire({ icon: "warning", title: "សូមរង់ចាំការទូទាត់ KHQR!" });
                    return;
                }
            ...
            if (received < currentTotal && !customerId) {
                Toast.fire({ icon: "error", title: "ជ្រើសរើសអតិថិជន ដើម្បីកត់ត្រាការជំពាក់!" });
                return;
            }
            ...
            if (received < currentTotal) {
                Swal.fire({
                    ...
                    confirmButtonText: 'បាទ/ចាស លក់ជំពាក់', cancelButtonText: 'បោះបង់'
                }).then(r => { if (r.isConfirmed) processCheckout(payType, received, customerId, false); });
            } else {
            ...
            $.ajax({
                ...
                success(res) {
                    if (res.status === 'success') {
                        ...
                        Swal.fire({
                            title: 'ជោគជ័យ',
                            text: res.message + (res.payment_status === 'Partial' ? ' (ជំពាក់)' : ''),
